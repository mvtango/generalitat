...
var maketext=function(child) {
	var a='<li><a  href="#">' + child.name + " " 
	+ "</a><div class=\"relation\">(resp.: " 
	+ (child.data["resp"] ? child.data["resp"] : "-")
	+ ' <a target="_blank" href="'+makeurl(child)+ '">mes informació'+
	"</a>)</div></li>";
	return(a);
}

var makeshort=function(txt) {
		txt=txt.replace(/Departament d(e la |'|e )/,"");
		txt=txt.replace(/Secretaria d(e la |'|e )/,"");		
...
function init(){
    ...
    var ht = new $jit.Hypertree({
      ...
      Tips: {
        ...
        {
            tip.innerHTML = node.name+"<br/> Resp.:"+(node.data["resp"] ? node.data["resp"] : '-');
        }
	  },
    ...
    for (i=0;i<json.children.length;i++) {
		if (json.children[i].id=="6") {
			presidencia=json.children[i];
		}
	}
    ht.loadJSON(presidencia);
