	...
	def extract(self, **args) :
		...
		for (k,v) in args.items() :
			if (len(v)==2) :
				...
				vm=v[1]
				vv=[]
				for e in self.select(vs) :
					vv.append(e.extract(**vm))
				r[k]=vv
			else :
				vv=self.select(v)
				if len(vv)==1 :
...
def myparser(base=etree.HTMLParser) :
...
class TreeScraper :
	...
	def __init__(self, ss,base=etree.HTMLParser,**kwargs) :
		if type(ss) in types.StringTypes :
			if len(ss)>0 and ss[0]=="<" :
				self.tree=etree.fromstring(ss,myparser(base))
			else :
				if kwargs:
					r=requests.get(ss,**kwargs)
					r.raise_for_status()
					self.tree=etree.fromstring(r.content,myparser(base))
				else :
					self.tree=etree.parse(ss,myparser(base))
		else :
			self.tree=etree.parse(ss,myparser(base))
